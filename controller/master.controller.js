...
// defect Master
exports.setDefect = async (req, res) => {
  try {
    const { flag, cond } = req.body;

    const data = await masterServices.setDefect(flag, cond);

    res.json({
      success: true,
      message: "Select defect data completed",
      data: data
    });
...
exports.saveDefect = async (req, res) => {
  try {
    const { data } = req.body;
    ...
    await masterServices.saveDefect(data);

    res.status(200).json({
      success: true,
      message: "Save defect data completed"
    });
...
exports.deleteDefect = async (req, res) => {
  try {
    ...
    await masterServices.deleteDefect(data);

    res.status(200).json({
      success: true,
      message: "Delete defect data completed"
    });
